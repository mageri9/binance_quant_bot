...
def test_market_fills_apply_adverse_slippage_and_commission():
    kernel = ExecutionKernel(ExecutionCosts(
        ...
        bid_ask_spread_rate=Decimal("0.0002"),
    ...
    assert buy.price == Decimal("100.2100")
    assert sell.price == Decimal("109.7690")
    assert kernel.realized_pnl(entry=buy, exit=sell, is_short=False) == Decimal("18.6980420")
...
def test_backtest_uses_execution_kernel_for_trade_return():
    ...
    exit = kernel.market_fill(side="sell", reference_price=110, amount=1)
